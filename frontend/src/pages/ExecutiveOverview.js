(function () {
  ...
  function WorkqueueTable({items, onCaseUpdate, onServiceRequest}) {
    ...
    function renderDetails(item) {
      function runCaseAction(action) {
        ...
        api.incidents.runCaseAction(item.caseId, {
          action,
  ...
  function ExecutiveOverviewApp() {
    ...
    async function load(selectedRange = range, refresh = false) {
      setLoading(true);
      try {
        const payload = await api.integrations.getExecutiveOverview(selectedRange, refresh);
        setData(payload);
    ...
    async function createServiceRequest(targetCase) {
      ...
      try {
        const ticket = await api.cases.saveTicket({
