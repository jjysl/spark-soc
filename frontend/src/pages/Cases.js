(function () {
  ...
  function TicketsBoardApp() {
    ...
    async function load() {
      const [ticketsData, blockData, actionData, escalationData, jiraData] = await api.cases.getDashboardData();
      setTickets(ticketsData || []);
      setBlockLog(blockData || []);
      setActionEvents(actionData || []);
      setEscalations(escalationData || []);
      setJiraStatus(jiraData || {configured: false, connected: false});
    ...
    async function saveTicket() {
      setSaving(true);
      try {
        await api.cases.saveTicket(editing, editing?.id ? editing : null);
        setEditing(null);
    ...
    async function blockIp() {
      ...
      setLastAction(null);
      try {
        const payload = await api.cases.blockIp({
          ip: editing.ip,
          country: editing.country || '',
          reason: editing.title || editing.id,
          source: 'manual',
          severity: editing.priority === 'p1' ? 'critical' : 'high',
          incident_id: editing.incidentLink || editing.id || '',
        });
        const fg = payload.fortigate || {ok: payload.status === 'blocked', message: payload.message || payload.status || 'FortiGate response received'};
        setLastAction(fg);
        setMessage(payload.message || `FortiGate blocklist updated for ${editing.ip}.`);
        setEditing({...editing, ipBlocked: true});
        await load();
      } finally {
        setBlocking(false);
      }
    ...
    async function escalate() {
      ...
      await api.cases.escalate({ticket_id: editing.id, to: editing.escalatedTo, reason: editing.escalationReason || '', analyst: editing.assignee || 'SOC'});
      setMessage(`Escalation registered for ${editing.id}.`);
      await load();
    }

    async function syncCurrentToJira() {
      if (!editing?.id) return;
      const payload = await api.cases.createJiraIssue(editing.id);
